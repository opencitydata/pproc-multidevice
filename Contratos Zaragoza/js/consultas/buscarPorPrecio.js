...
var query = 'SELECT DISTINCT ?uri min(?titulo) as ?Titulo ?nombre ucase(replace(replace(replace(?cif," ",""),"-",""),"/.","")) as ?Cif min(?servicioGestor) as ?ServicioGestor ?id ?precio\
		WHERE {\
...
$.getJSON(SPARQL_ENDPOINT + '?query=' + encodeURIComponent(query) + '&format=application%2Fsparql-results%2Bjson&timeout=0')
	.success(function(data) {
		...
		table += '<tr> <td><h2>Título</h2></td> <td><h2>Empresa</h2></td> <td><h2>Servicio Gestor</h2></td> <td><h2>Precio</h2></td> </tr>';
		...
		for (var i = 0; i < data.results.bindings.length; i++) {
			...
			table += '<tr><td>' + '<a href='+feature.uri.value+'>'+feature.Titulo.value+'</a></td> <td>' + '<a href="adjudicadosCIF.html?cif='+cif.value+'">'+nombre.value+'</a></td>  <td>' + '<a href="adjudicadosPorServicioGestor.html?servicio='+id.value+'">'+servicioGestor.value+'</a></td><td>' + feature.precio.value + '</td></tr>';
			}
		//Se cierra la tabla
		table += '</table>';
