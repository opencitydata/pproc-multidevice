...
$.getJSON(SPARQL_ENDPOINT + '?query=' + encodeURIComponent(query) + '&format=application%2Fsparql-results%2Bjson&timeout=0')
   .success(function(data) {
		...
		for (var i = 0; i < data.results.bindings.length; i++){
			feature = data.results.bindings[i];
			var x = i+1;
			var gauge = "fillgauge"+x+"";
			loadLiquidFillGauge(gauge, Math.round(100 * feature.Precio.value/feature.presupuesto.value));
			//loadLiquidFillGauge(gauge, 100 * feature.adjudicadoPor.value/feature.presupuesto.value);
		}
